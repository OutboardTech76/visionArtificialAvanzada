...
class Tracking:
    ...
    def trackObject(self, detectedPoses: List[int], detectedLabels: List[str], numFrame: int) -> int:
        """
        Method to manage tracked object
        Args:
            detectedPoses -> List of lists of 4 elements with objects position [x,y,w,h]
            detectedLabels -> List with labels
...
def parser():
    ...
    ap.add_argument("-w", "--weights", default="yolov3.weights", help="YOLO weights. Default: yolov3.weights")
    ap.add_argument("-f", "--file", default="yolov3.cfg", help="YOLO configuration file (.cfg). Default: yolov3.cfg")
    ap.add_argument("-l", "--labels", default="coco.names", help="Labels file. Default: coco.names")
...
def showNumObj(img: Image, obj: int, people: int) -> None:
    """
    Shows number of objects/people in every frame
    Args:
        img
        obj -> numObjects
        people -> numPeople
    ...
    cv2.putText(img, "People: {}".format(people), (10, 40), cv2.FONT_HERSHEY_SIMPLEX, 0.5, (0, 0, 255), 2 )
    cv2.putText(img, "Objects: {}".format(obj), (10, 60), cv2.FONT_HERSHEY_SIMPLEX, 0.5, (0, 0, 255), 2)

def showTotalObj(img: Image, obj: int, people: int) -> None:
    """
    Shows number of objects/people 
    Args:
        img
        obj -> numObjects
        people -> numPeople
    ...
    cv2.putText(img, "People: {}".format(people), (200, 40), cv2.FONT_HERSHEY_SIMPLEX, 0.5, (0, 0, 255), 2 )
    cv2.putText(img, "Objects: {}".format(obj), (200, 60), cv2.FONT_HERSHEY_SIMPLEX, 0.5, (0, 0, 255), 2)
...
if __name__ == '__main__':
    ...
    while(cap.isOpened()):
        ...
        showNumObj(frame, numObjects, numPeople)
        ...
        totalPeople = sum([1 if obj.checkObject() and obj.label == "person" else 0 for obj in trackedObjectList])
        totalObjects = sum([1 if obj.checkObject() and obj.label != "person" else 0 for obj in trackedObjectList])
        showTotalObj(frame, totalObjects, totalPeople)
        cv2.imshow('a',frame)
